...
def assimilate(s2_refs, arc_refs, s2_errs, pheo_samples, bio_samples, soil_samples, orig_bios, neighbours, num_ens=50):
    ...
    def process_valid_index(i):
        ...
        mean_ref = jnp.sum(arc_refs[:, :, best_candidate] * weight[None, None], axis=(2))
        ...
        return post_bios, post_bios_unc, mean_bio_scales, mean_pheo, mean_soils, mean_ref, best_candidate
    ...
    def process_invalid_index(i):
        ...
        # return post_bios, post_bios_unc, mean_bio_scales, mean_pheo, mean_soils
        mean_ref = jnp.zeros((s2_refs.shape[0], s2_refs.shape[1]))
        best_candidate = jnp.zeros(num_ens, dtype=jnp.int32)
        return post_bios, post_bios_unc, mean_bio_scales, mean_pheo, mean_soils, mean_ref, best_candidate
...
# @partial(jax.jit, static_argnums=(8,))
# def assimilate(ref, s2_refs, band_errs, pheo_samples, bio_samples, soil_samples, orig_bios, neighbours, num_ens=50):
#     n_samples = ref.shape[2]

#     orig_bios_dim1 = orig_bios.shape[0]
#     orig_bios_dim2 = orig_bios.shape[1]

#     mask = jnp.isfinite(ref).any(axis=(0, 1))

#     def true_branch(i):
#         ann_neighbours = neighbours[i]
#         en_refs = s2_refs[:, :, ann_neighbours]
#         s2_ref = ref[:, :, i]
#         unc = band_errs[:, :, i]
#         diff = s2_ref[..., None] - en_refs
...
#         weight = div(1., l2_distance)
#         weight = div(weight, weight.sum())
#         # weight = (1.0 / l2_distance) * (1.0 / l2_distance.sum())

#         mean_pheo = jnp.sum(pheo_samples[best_candidate] * weight[:, None], axis=0)
#         mean_soils = jnp.sum(soil_samples[best_candidate] * weight[:, None], axis=0)
#         mean_bio_scales = jnp.sum(bio_samples[best_candidate] * weight[:, None], axis=0)
        
#         mean_ref = jnp.sum(s2_refs[:, :, best_candidate] * weight[None, None], axis=(2))

#         post_bios = jnp.sum(orig_bios[:, :, best_candidate] * weight[None, None], axis=(2))
#         post_bios_unc = jnp.sqrt(jnp.sum((orig_bios[:, :, best_candidate] - post_bios[:, :, None])**2 * weight[None, None] * num_ens / (num_ens-1), axis=2))
        
#         post_bios = post_bios.astype(jnp.int32)
#         post_bios_unc = post_bios_unc.astype(jnp.int32)
...
#         mean_pheo = jnp.zeros(pheo_samples.shape[1])
#         mean_soils = jnp.zeros(soil_samples.shape[1])
#         mean_ref = jnp.zeros((ref.shape[0], ref.shape[1]))
#         best_candidate = jnp.zeros(num_ens, dtype=jnp.int32)
#         return post_bios, post_bios_unc, mean_bio_scales, mean_pheo, mean_soils, mean_ref, best_candidate

#     def single_sample(i):
#         return cond(mask[i], i, true_branch, i, false_branch)

#     results = jax.vmap(single_sample)(jnp.arange(n_samples))

#     return tuple(jnp.stack(result) for result in results)

#     post_bio_tensor = jnp.stack(results[0])
#     post_bio_unc_tensor = jnp.stack(results[1])
#     post_bio_scale_tensor = jnp.stack(results[2])
#     post_pheo_tensor = jnp.stack(results[3])
#     post_soil_tensor = jnp.stack(results[4])
#     mean_ref = jnp.stack(results[5])
#     best_candidate = jnp.stack(results[6])

#     return post_bio_tensor, post_bio_unc_tensor, post_bio_scale_tensor, post_pheo_tensor, post_soil_tensor, mean_ref, best_candidate
